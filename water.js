...
function Water() {
  ...
  this.sphereShader = new GL.Shader(vertexShader, '\
    ...
    float volumeInSphere(vec3 center) {\
      vec3 adjCenter = vec3(center.x, center.y - waterLevel, center.z);\
      vec3 toCenter = vec3(coord.x * 2.0 - 1.0, 0.0, coord.y * 2.0 - 1.0) - adjCenter;\
      float t = length(toCenter) / radius;\
      float dy = exp(-pow(t * 1.5, 6.0));\
      float cy = adjCenter.y;\
      float ymin = min(0.0, cy - dy);\
      float ymax = min(max(0.0, cy + dy), ymin + 2.0 * dy);\
...
Water.prototype.moveSphere = function(oldCenter, newCenter, radius) {
  ...
  this.textureB.drawTo(function() {
    ...
    this_.sphereShader.uniforms({
      ...
      newCenter: newCenter,
      radius: radius,
      waterLevel: (typeof this_.waterLevel !== 'undefined') ? this_.waterLevel : 0.0
    }).draw(this_.plane);
  });
  this.textureB.swapWith(this.textureA);
};
